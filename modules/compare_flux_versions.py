...
def flux_of_different_versions():
    # Streamlit 页面配置
    st.title("'文生图' 标注创建器")
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            model1_image_col = st.selectbox("Model 1 Image URL", options=column_names, index=get_column_index(df, 'a800_05'))

        with col2:
            all_prompt_cn_col = st.selectbox("Chinese All_prompt", options=column_names, index=get_column_index(df, 'All_prompts_cn'))
            model2_image_col = st.selectbox("Model 2 Image URL", options=column_names, index=get_column_index(df, 'a800_07'))

        with col3:
            all_prompt_en_col = st.selectbox("English All_prompt", options=column_names, index=get_column_index(df, 'All_prompts'))
            model3_image_col = st.selectbox("Model 3 Image URL", options=column_names, index=get_column_index(df, 'a800_10'))
        ...
        column_names = [original_image_col, all_prompt_cn_col, all_prompt_en_col,
                        model1_image_col, model2_image_col, model3_image_col]
        ...
        if st.button("启动"):
            ...
            with st.spinner('运行中...'):
                ...
                project = label_manager.create_project(title='文生图（多模型比较）', task_type='flux_models_compare')
                ...
                for index, (_, row) in enumerate(tqdm(selected_df.iterrows(), total=total_rows)):
                    ...
                    task_data = {
                        'data': {
                            'url': row[original_image_col],
                            'a800_05': row[model1_image_col],
                            'a800_07': row[model2_image_col],
                            'a800_10': row[model3_image_col],
...
def flux_of_different_versions_text():
    # Streamlit 页面配置
    st.title("'文生图' 标注创建器")
    label_config['labelstudio']['url'] = 'http://localhost:20003'
    label_config['labelstudio']['external_port'] = '20003'
    ...
    if uploaded_file is not None:
        ...
        with col1:
            original_image_col = st.selectbox("Original Image URL", options=column_names, index=get_column_index(df, 'url'))
            model1_image_col = st.selectbox("Model 1 Image URL", options=column_names, index=get_column_index(df, 'a800_05'))
            model4_image_col = st.selectbox("Model 4 Image URL", options=column_names, index=get_column_index(df, 'a800_15'))            
        ...
        # 选择的 DataFrame 范围
        column_names = [original_image_col, all_prompt_cn_col, all_prompt_en_col,
                        model1_image_col, model2_image_col, model3_image_col, model4_image_col]
        selected_df = df.iloc[start_row:end_row+1][column_names]
        ...
        if st.button("启动"):
            ...
            with st.spinner('运行中...'):
                ...
                project = label_manager.create_project(title='文生图（多模型比较）', task_type='flux_models_compare_text')
                project_url = label_manager.get_project_data_url(project)
                ...
                for index, (_, row) in enumerate(tqdm(selected_df.iterrows(), total=total_rows)):
                    ...
                    task_data = {
                        'data': {
                            'url': row[original_image_col],
                            'model1_name': model1_image_col,
                            'model2_name': model2_image_col,
                            'model3_name': model3_image_col,
                            'model4_name': model4_image_col,                            
                            'model1_path': row[model1_image_col],
                            'model2_path': row[model2_image_col],
                            'model3_path': row[model3_image_col],
                            'model4_path': row[model4_image_col],
                            'All_prompts': convert_to_html_and_escape_xml(row[all_prompt_cn_col]),
                        }
                    }            
