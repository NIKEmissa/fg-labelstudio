...
def compute_matched_pairs(log1, log2, debug=False):
    ...
    for idx, (item1, item2, iou) in enumerate(matched_pairs):
        ...
        tag1 = ann1.get("tag", "UNKNOWN")
        tag2 = ann2.get("tag", "UNKNOWN")
        if re.sub(r'\d+$', '', tag1) == re.sub(r'\d+$', '', tag2):
            global_group += 1
            ...
            if debug:
                clean_tag1 = re.sub(r'\d+$', '', tag1)
                clean_tag2 = re.sub(r'\d+$', '', tag2)
                st.write(f"### Debug: 丢弃匹配对 - Annotator1 tag: {tag1} 与 Annotator2 tag: {tag2} 不一致（忽略数字后缀后分别为 {clean_tag1} 与 {clean_tag2}）")
...
def get_primary_result(log):
    """
    从单个 log 中提取一级标签，去除尾部数字，去重后排序并合并为逗号分隔的字符串。
    """
...
def compare_primary_labels_overall(rst1, rst2):
    """
    对比两个标注结果文件中的所有 log（根据 log id 匹配）。
    生成一个包含 id、结果一、结果二 的表格，对于结果不同的单元格用红色高亮显示，
    并分别统计【包含缺失】与【排除缺失及空白】的一致率，同时展示排除了无效数据后的表格。
    """
    # 构建 id 到 log 的映射
    ...
    # 定义高亮函数：对于“结果一”和“结果二”列，当二者不相等时高亮背景为红色
    def highlight_diff(row):
        styles = {}
        if row["结果一"] != row["结果二"]:
            styles["结果一"] = "background-color: red"
            styles["结果二"] = "background-color: red"
        else:
            styles["结果一"] = ""
            styles["结果二"] = ""
        styles["id"] = ""
    ...
    st.write("### 一级标签整体对比结果（包含缺失）")
    st.markdown(styled_df.to_html(escape=False, index=False), unsafe_allow_html=True)
    
    # 统计【包含缺失】：直接对所有记录计算一致率
    ...
    st.write(f"【包含缺失】一致率：{consistency_overall:.2f}% （匹配：{match_overall} / 总数：{total_overall}）")
    st.write(f"【排除缺失及空白】一致率：{consistency_valid:.2f}% （匹配：{match_valid} / 有效总数：{total_valid}）")
    
    # 追加展示排除了无效数据后的过滤表格
    if total_valid > 0:
        styled_df_valid = df_valid.style.apply(highlight_diff, axis=1)
        st.write("### 排除无效数据（缺失和空白）后的对比结果")
        st.markdown(styled_df_valid.to_html(escape=False, index=False), unsafe_allow_html=True)
    else:
        st.write("没有有效数据用于展示排除无效数据后的表格。")
...
def compare_anno_results():
    ...
    # 模式选择：完整对比 vs 一级标签对比
    compare_mode = st.radio("选择对比模式", ["完整对比", "一级标签对比"])
    ...
    if uploaded_file1 is not None and uploaded_file2 is not None:
        ...
        if compare_mode == "完整对比":
            ...
            # 一级标签对比模式：直接对比所有 log，展示整体表格以及一致率统计
