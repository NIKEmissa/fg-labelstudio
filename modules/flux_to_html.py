    ...
    if uploaded_files:
        ...
        if st.button("Run"):
            ...
            with st.spinner("正在生成HTML文件，请稍候..."):
                # 选择一个特定的 img_id
                def select_specific_img_id(df, img_id):
                    return df[df['img_id'] == img_id]  # 过滤出同一个 img_id 的所有条目

                # 新增函数，生成整理表格
                def generate_summary_table(filtered_df):
                ...
                for img_id in unique_img_ids:
                    filtered_df = select_specific_img_id(df, img_id)
                    summary_table, img_seq_size_seed_pairs, guide_values = generate_summary_table(filtered_df)
                    generate_html_page(summary_table, img_seq_size_seed_pairs, guide_values, img_id)
