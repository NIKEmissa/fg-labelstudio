...
def process_data(data, model, port, output_dir, retry_count=0):
    ...
    try:
        ...
        all_prompt = data["All_prompts"]
        all_gt = data["All_gt"]
        user_prompt = USER_PROMPT.replace(ALL_GPT_PROMPT_KEYWORD, all_prompt).replace(ALL_GT_KEYWORD, all_gt)
        request_data = {
            "model": model,
            "messages": [
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="output/batch_merge")
    parser.add_argument("--failed_dir", type=str, default="output/fail_merge")
