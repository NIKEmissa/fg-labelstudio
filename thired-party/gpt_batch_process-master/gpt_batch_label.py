...
# 日志配置
logging.basicConfig(
    filename='log/data_description.log',
    level=logging.INFO,
...
def process_data(data, model, port, output_dir, retry_count=0):
    ...
    try:
        ...
        # 如果是 URL 则直接使用，如果是本地图片路径则编码
        if data['url'].startswith('http'):
            image = data['url']
        else:
            encoded_image = encode_image(data['url'])
            if encoded_image:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="output/batch_label")
    parser.add_argument("--failed_dir", type=str, default="output/fail_label")
